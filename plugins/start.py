...
async def start(client, message):
    if not await checkdb.is_user_exist(message.from_user.id):
        ...
        rm = InlineKeyboardMarkup([[InlineKeyboardButton("✨ Update Channel", url="https://t.me/WarriorUnitsBots")]])
...
async def stream_start(client, message):
    ...
    encoded_url = f"{LINK_URL}?WarriorPlayer={link}"
...
async def link_start(client, message):
    if not message.text.startswith(LINK_URL):
        return
    link_part = message.text[len(LINK_URL + "?WarriorPlayer="):].strip()
    try:
